        ...
        if status == 'completed':
            ...
            for index,row in chapters_df.iterrows():
                with st.expander(row['gist']):
                    ...
                    st.write(translator.translate(row['summary'], dest='ta').text)
...
if start_button.button("Record audio"):
    ...
    try:
        ...
        chunks = [word_list[i:i+10] for i in range(0, len(word_list), 10)]
        chunk_scores = {}
        for chunk in chunks:
        ...
        summarized_chunks = nlargest(5, chunk_scores, key=chunk_scores.get)
        summary = '. '.join(summarized_chunks)

        st.write("Summary:", summary)
        translated_text = translator.translate(summary, dest='es').text
